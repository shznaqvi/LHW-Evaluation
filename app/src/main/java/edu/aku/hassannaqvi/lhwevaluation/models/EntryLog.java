...
import static edu.aku.hassannaqvi.lhwevaluation.core.MainApp._EMPTY_;
...
public class EntryLog extends BaseObservable implements Observable {
    ...
    public void populateMetaHHForm() {
        ...
        setPsuCode(MainApp.hhForm.getLhwCode());
        setHhid(MainApp.hhForm.getKhandandNo());
        setiStatus(MainApp.hhForm.getiStatus());
        setiStatus96x(MainApp.hhForm.getiStatus96x());
    ...
    public void populateMetaLHWForm() {

        setProjectName(PROJECT_NAME);
        setUuid(MainApp.LHWForm.getUid());  // not applicable in Form table
        setUserName(MainApp.user.getUserName());
        setSysDate(MainApp.LHWForm.getSysDate());
        setEntryDate(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH).format(new Date().getTime()));
        setPsuCode(MainApp.LHWForm.getCluster());
        //setHhid(MainApp.LHWForm.getHhid());
        setiStatus(MainApp.LHWForm.getiStatus());
        setiStatus96x(MainApp.LHWForm.getiStatus96x());
        setAppver(MainApp.appInfo.getAppVersion());
        setEntryType("LHWForm");
        setDeviceId(MainApp.deviceid);
